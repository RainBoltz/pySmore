...
def learner():
    ...
    for i in range(1, globalVariables['update_times']+1):
        user, user_idx = globalVariables['graph'].draw_user()
        item, item_idx = globalVariables['graph'].draw_item(user)
        weight = globalVariables['graph'].get_weight(user, item)
...
userEmbed = embedding.create_embeddings_unsafe(amount=globalVariables['graph'].get_user_amount())
itemEmbed = embedding.create_embeddings_unsafe(amount=globalVariables['graph'].get_item_amount())
